...
class ConditionalUNet(nn.Module):
    def __init__(self):
        ...
        # Class embedding
        self.class_emb = nn.Embedding(num_embeddings=10, embedding_dim=time_emb_dim)
    ...
    def forward(self, x, t, labels):
        ...
        # Down
        x1 = self.down1_1(x, emb)
        x1 = self.down1_2(x1, emb)
        p1 = self.pool1(x1)
        
        x2 = self.down2_1(p1, emb)
        x2 = self.down2_2(x2, emb)
        p2 = self.pool2(x2)

        # Bot
        bot = self.bot1(p2, emb)
        bot = self.attn(bot)
        bot = self.bot2(bot, emb)

        # Up
        u1 = self.up1(bot)
        u1 = torch.cat([u1, x2], dim=1)
        u1 = self.up_conv1_1(u1, emb)
        u1 = self.up_conv1_2(u1, emb)
        
        u2 = self.up2(u1)
        u2 = torch.cat([u2, x1], dim=1)
        u2 = self.up_conv2_1(u2, emb)
        u2 = self.up_conv2_2(u2, emb)
